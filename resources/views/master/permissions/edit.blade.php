<x-app-layout>
    <div class="container w-1/2 rounded mx-auto my-4 p-6 sm:px-20 bg-white border-b border-gray-200 h-auto">
        <h1 class="text-center text-xl font-extrabold text-gray-500 my-2">{{__('Edit permission')}}</h1>
        <form action="{{route('permissions.update',$permission->id)}}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div class="gird-col-span-1 md:col-span-4">
                    ...
                    <x-jet-input name="name" type="text" class="mt-1 block w-full" autocomplete="name" value="{{$permission->name}}" />
                    <x-jet-input-error for="name" class="mt-2" />
                </div>

                <div class="gird-col-span-1 md:col-span-4">
                    <x-jet-label for="permission" value="{{ __('Route') }}" />
                    <x-jet-input name="permission" type="text" class="mt-1 block w-full" autocomplete="permission" value="{{$permission->permission}}"/>
                ...
                <div class="col-span-1 flex items-center justify-items-end">
                    ...
                    <input type="submit" value="{{__('update')}}" permission="button"
