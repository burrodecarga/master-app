<x-app-layout>
    ...
    <div class="container w-50 rounded mx-auto my-4 p-6 sm:px-20 bg-white border-b border-gray-200 h-auto">
        ...
        <div class="flex items-center justify-between">
            <div>
               <h1 class="text-center text-xl font-bold text-gray-500 my-2">{{__('List of permissions')}}</h1>

            </div>
            <div>
                <a href="{{route('permissions.create')}}" title="{{__('add new record')}}" class="btn btn-outline-primary hover:bg-white hover:text-white"><i class="fa fa-plus-circle"></i></a>
            </div>
        </div>

        <table id="permissions" class="table table-striped table-bordered text-sm" style="width:100%">
            <thead>
                <tr>
                    <th width="50%">Name</th>
                   ...
                   <tr>
                    ...
                    <td class="text-center flex items-center justify-between">
                        ...
                        <form id="delete-form{{$permission->id}}" action="{{route('permissions.destroy',$permission->id)}}"
                            method="POST"  class="borrar-permission">
                            @csrf
                            @method('DELETE')
                            <button type="submit" permission="button"
...
<script>
   ...
   $('.borrar-permission').submit(function(e){
      e.preventDefault()
            Swal.fire({
  title: 'Are you sure?',
