...
<nav x-data="{open: false }" class="bg-gray-800">
        ...
        <div class="relative flex items-center justify-between h-16">
            ...
            <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                <a href="/" class="flex-shrink-0 flex items-center">
                    <img class="block lg:hidden h-8 w-auto" src="{{asset('assets/logo/10.png')}}" alt="Rokave">
                    <img class="hidden lg:block h-8 w-auto" src="{{asset('assets/logo/10.png')}}" alt="Rokave">
                </a>
    ...
    <div x-show="open" x-on:click.away="open=false" class="sm:hidden" id="mobile-menu">
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            @foreach ($navLink as $link)
            <a href="{{$link['href']}}"
                class="bg-gray-900 text-white block px-3 py-2 rounded-md text-base font-medium hover:bg-gray-50 hover:text-gray-500"
                aria-current="page" active="{{request()->routeIs($link['name'])}}">{{ __($link['text']) }}</a>
            @endforeach
